...
__all__ = ['NDNSerial', 'ChannelMode', 'OnOffState', 'Tracking']
...
class NDNSerial(object):
    ...
    class Channel(object):
        ...
        def __init__(self, serial_, channel_number):
            ...
            """
            super(NDNSerial.Channel, self).__init__()
            self.__serial = serial_
            self.number = channel_number

        @property
        def current(self):
            result = self.__serial.send_receive(r"ISET{0}?\r\n".format(self.number), fixed_length=6)
        ...
        def current(self, value):
            self.__serial.send(r"ISET{0}:{1:5.3f}\r\n".format(self.number, value))

        @property
        def voltage(self):
            return float_or_none(self.__serial.send_receive(r"VOUT{0}?\r\n".format(self.number), fixed_length=5))

        @voltage.setter
        def voltage(self, value):
            self.__serial.send(r"VSET{0}:{1:05.2f}\r\n".format(self.number, value))
        ...
        def output_current(self):
            ...
            :rtype: float or None
            """
            result = self.__serial.send_receive(r"IOUT{0}?\r\n".format(self.number))
        ...
        def output_voltage(self):
            ...
            :rtype: float or None
            """
            result = self.__serial.send_receive(r"VOUT{0}?\r\n".format(self.number))
            return float_or_none(result)
    ...
    class Memory(object):
        ...
        def __init__(self, serial_, memory_number):
            super(NDNSerial.Memory, self).__init__()
            self.__serial = serial_
    ...
    class OnOffButton(object):
        ...
        def __init__(self, serial_, on_command, off_command):
            super(NDNSerial.OnOffButton, self).__init__()
            self.__serial = serial_
            self._on = on_command
    ...
    class Serial(object):
        ...
        def __init__(self, port, debug=False):
            super(NDNSerial.Serial, self).__init__()

            self.debug = debug
            self.port = serial.Serial(port, 9600, timeout=0.3)
    ...
    def __init__(self, port, debug=False):
        super(NDNSerial, self).__init__()

        self.__serial = NDNSerial.Serial(port, debug)

        # Channels: adjust voltage and current,  discover current output voltage.
        self.channels = [NDNSerial.Channel(self.__serial, i) for i in range(1, 3)]

        # Memory recall/save buttons 1 through 5
        self.memories = [NDNSerial.Memory(self.__serial, i) for i in range(1, 6)]

        # Second column buttons
        self.beep = NDNSerial.OnOffButton(self.__serial, "BEEP1", "BEEP0")
        self.output = NDNSerial.OnOffButton(self.__serial, r"OUTPUT1\r\n", r"OUTPUT0\r\n")
        self.over_current_protection = NDNSerial.OnOffButton(self.__serial, "OCP1", "OCP0")
        self.over_voltage_protection = NDNSerial.OnOffButton(self.__serial, "OVP1", "OVP0")
    ...
    def model(self):
        ...
        :rtype: str
        """
        return self.__serial.send_receive(r"*IDN?\r\n")
